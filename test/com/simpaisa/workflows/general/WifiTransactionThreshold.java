package com.simpaisa.workflows.general;
...
public class WifiTransactionThreshold {
    @Test(enabled = true, groups = {"wifi","workflow"})
    public void make_transasction_wifi() throws IOException, URISyntaxException {

        //Note: Threshold applied, sufficient balance , Product, Operator, mobile all in place(properties)
        ...
        Assert.assertEquals(output3.getValue("message"), "0-Transaction Successful");

        //TODO: Tentry in ransaction and recursion table must be = 1


        // TODO:  OUT OF SCOPE: unsubscribe user

        // TODO: repeat STep 01 repeated , we'll get response "Transaction periodic threshold exceeded"

        SimpaisaAPIRequests request4 = new SimpaisaAPIRequests();
        System.out.println(" STEP01: === Initiating Make Transaction Request (POST) ");
        SimpaisaResponseParser respons41 = request4.wifi_make_transaction();

        SimpaisaHashMapper output4 = new SimpaisaHashMapper(response1.misc1());
        System.out.println("  < Display Request Responses >");

        System.out.println("Response Body Message : " + output1.getValue("message"));
        Assert.assertEquals(output4.getValue("message"), "Transaction periodic threshold exceeded");
